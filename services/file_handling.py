...
def _get_part_text(text: str, start: int, size: int) -> tuple[str, int]:
    # Определяем списко знаков препинания, которые могут быть окончанием страницы
    p_marks: list[str] = [',', '.', '!', ':', ';', '?']

    # Определяем кусок текста по заданным параметрам
    chunk: str = text[start:start+size]

    # Если кусок заканчивается на точку и следующий символ не точка
...
text = '''РЭЙ БРЕДБЕРИ ПРЕДОСТЕРЕГАЕТ
   ...
   Сам Рэй Бредбери вносит заметный вклад в борьбу именно за такого'''

print(*_get_part_text(text, 187, 700), sep='\n')
